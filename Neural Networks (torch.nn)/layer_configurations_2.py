# auto updated code based of previous version's output
...
def test_layer(layer, input_tensor):
    try:
        # Run the layer and handle cases where the output might be a tuple (e.g., RNN layers)
        output = layer(input_tensor)
        if isinstance(output, tuple):
            output = output[0]  # Extract the tensor part of the output
